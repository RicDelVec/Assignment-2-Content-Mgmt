...
function twentytwenty_register_styles() {
	...
	// Add custom CSS.
	wp_enqueue_style( 'assignment1-style', get_template_directory_uri() . '/rdv.css');
}
...
function photography_init(){
	$args = array(
		...
		'label' => 'Photography',
		// public - whether a post type is intended to be used publicly either via the admin interface or by front-end users. WordPress sets this to false by default. Here we set it to true as we do want our custom post type to display publicly
		'public' => true,
		// show_ui - Generates a default UI for managin this post type in the admin. You can set this to true or false. For the sake of usability, a UI in the admin area is always a good thing.
		'show_ui' => true,
		...
		// hierarchical - Whether the post type is heirarchical (eg page). Or in laymen's terms, whether or not you can declare a parent page, child page, etc. of the post type. This is mainly intended for pages. Here we declare it false so there's no need to worry about it for our example
		'hierarchical' => false,
		...
		// supports - This is usually an array of features the custom post type will support. Here we have quite a long list. These will tie into the admin area (this is basically a list of of all of them, but you don't need to include them all) - don't forget page-attributes
		'supports' => array(
			'title',
			'editor',
			'excerpt',
...
function photo_shortcode(){
	add_shortcode('photo_shortcode', 'photography');
}
add_action('init', 'photo_shortcode');